  ...
  this.createWriteStream = function(monitorDir, socket){
    ...
    var onFile = function(setupInfo){
      ...
      return function(data){
        ...
        buff += data.length || 0;
        //data (buffer, state, filename)
        socket.emit('fileDataWrite', {
          key: uid,
          buffer: data,
          state: info.state,
        ...
        console.log('sending ' + info.filename + " " + ((buff/info.totalSize).toPrecision(3) * 100) + "%")
      };
    }.bind(this);
    ...
    watcher(monitorDir, function(filename){
      ...
      if(fs.lstatSync(filename).isFile()){
        ...
        if(shortname !== path.basename(self.currentlySyncing)){
          ...
          fs.createReadStream(filename)
            .pipe(through(onFile(fileInfo), onEnd(fileInfo)));
