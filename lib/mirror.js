...
var fs = require('fs'), //filesystem
    watch = require('walkie-watchie'), //filesystem watcher
...
var Mirror = function(key) {
    ...
    this.syncLocalFolders = function(slave, master) {
        ...
        watcher.on("create", function(path, stats) {
            ...
            //generate the save path wrt our slave directory
                var savePath = slave + '/' + utils.pathDif(path, master);
    ...
    this.createWriteStream = function(monitorDir, socket) {
        ...
        var tg = new ThrottleGroup({
            rate: maxTransferSpeed
        });
        //var throttler = new Throttle( (1024*3) * 1024);
        //   var emitOverload = function(){
        //   console.log(arguments);
        //   Function.prototype.call(this, socket.emit, arguments);
        // };
        //socket.emit = emitOverload;
        ...
        console.log('watching ' + monitorDir);
        var watcher = watch(monitorDir, {
            changeDelay: 100,
            moveDelay: 100
        });
    ...
    this.createReadStream = function(monitorDir, socket) {
        ...
        socket.on('move', function(data) {
            //{oldPath:oldPath, newPath:newPath, isDir: isDir}
            var relOldPath = utils.pathDif(data.oldPath, slave),
                relNewPath = utils.pathDif(data.newPath, slave);

            fs.rename(relOldPath, relNewPath, function(err) {
                if (err) console.error(err);
            });
        });
        socket.on('delete', function(data) {
            //{oldPath:oldPath, newPath:newPath, isDir: isDir}
            var relDeletePath = utils.pathDif(data.oldPath, slave);

            fs.unlink(relDeletePath, function(err) {
                if (err) console.error(err);
                console.log('successfully deleted ' + relDeletePath);
            });
        });
    };
};
